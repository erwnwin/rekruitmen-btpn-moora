...
class M_cs extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT * FROM penilaian_cs WHERE id_akun_calon_pegawai = '$id_alternatif' AND id_kriteria = '$id_kriteria';");
    ...
    public function get_nilai_setiap_alternatif()
    {
        ...
        JOIN penilaian_cs ON akun_calon_pegawai.id_akun_calon_pegawai = penilaian_cs.id_akun_calon_pegawai;");
    ...
    {
        $query = $this->db->query("SELECT SQRT(SUM(POWER(total_nilai, 2))) AS nilai_pembagian FROM penilaian_cs WHERE id_kriteria='$id_kriteria';");
    ...
    public function pembobotan_nilai($id_kriteria) // pembobotan nilai
    {
		...
		FROM penilaian_cs JOIN (SELECT SQRT(SUM(POWER(total_nilai, 2))) AS nilai_pembagian FROM penilaian_cs WHERE fk_id_kriteria='$id_kriteria') AS bobot_nilai 
		JOIN kriteria ON kriteria.id_kriteria = penilaian_cs.id_kriteria 
        WHERE kriteria.id_kriteria='$id_kriteria' GROUP BY penilaian_cs.id_akun_calon_pegawai");
