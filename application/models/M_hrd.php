...
class M_hrd extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT * FROM penilaian_hrd WHERE id_akun_calon_pegawai = '$id_alternatif' AND id_kriteria = '$id_kriteria';");
    ...
    public function get_nilai_setiap_alternatif()
    {
        ...
        JOIN penilaian_hrd ON akun_calon_pegawai.id_akun_calon_pegawai = penilaian_hrd.id_akun_calon_pegawai;");
    ...
    {
        $query = $this->db->query("SELECT SQRT(SUM(POWER(total_nilai, 2))) AS nilai_pembagian FROM penilaian_hrd WHERE id_kriteria='$id_kriteria';");
    ...
    public function pembobotan_nilai($id_kriteria) // pembobotan nilai
    {
		...
		FROM penilaian_hrd JOIN (SELECT SQRT(SUM(POWER(total_nilai, 2))) AS nilai_pembagian FROM penilaian_hrd WHERE fk_id_kriteria='$id_kriteria') AS bobot_nilai 
		JOIN kriteria ON kriteria.id_kriteria = penilaian_hrd.id_kriteria 
        WHERE kriteria.id_kriteria='$id_kriteria' GROUP BY penilaian_hrd.id_akun_calon_pegawai");
